...
const SecurityAssistant = () => {
  ...
  const [securityAdvice, setsecurityAdvice] = useState("");
  ...
  // 🔹 Fetch security Advice from Flask Backend
  const fetchsecurityAdvice = async () => {
    setIsLoading(true);
    try {
      // Update to use the correct endpoint and add error handling
      const userQuery = userInputRef.current?.value || "Give me cybersecurity advice";
      console.log("Sending query to backend:", userQuery);
      
      const response = await fetch("http://localhost:5000/security-advice", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch security advice: ${response.status} - ${errorText}`);
      }
      ...
      if (data.detailed_advice) {
        setsecurityAdvice(data.detailed_advice);
        
        // Make the avatar repeat the security advice using REPEAT task type
        if (avatar) {
          await avatar.speak({ 
      ...
      console.error("❌ Error fetching security advice:", error);
      
      // Provide more detailed error info
      const errorMessage = `Error: ${error.message}. Please check if the server is running at http://localhost:5000`;
      console.error(errorMessage);
      
      // Use sample security advice if the backend call fails
      const sampleAdvice = getSamplesecurityAdvice();
      setsecurityAdvice(sampleAdvice);
  ...
  // 🔹 Sample security advice function
  const getSamplesecurityAdvice = () => {
    const adviceOptions = [
      ...
      "If you're dealing with workplace discrimination, know that employment laws protect you against unfair treatment based on race, gender, disability, or other protected categories. Document any incidents, report them through the appropriate channels, and consider seeking security counsel if the issue persists.",
      
      "Considering starting a business? Make sure to choose the right security structure—LLC, sole proprietorship, or corporation—based on your needs. Each structure has different liability protections and tax implications, so consulting a business attorney can help you set up properly.",
      
      "If you've been accused of a crime, it's crucial to exercise your right to remain silent and seek security representation immediately. A criminal defense attorney can guide you through the security process and ensure your rights are protected.",
      
      "When dealing with family law matters like divorce or child custody, mediation can sometimes be a more amicable and cost-effective solution than court battles. Understanding your security rights and obligations early on can help you make informed decisions."
    ];
  ...
  const handleVoiceInputComplete = () => {
    SpeechRecognition.stopListening();
    if (transcript) {
      fetchsecurityAdvice(); // Use the voice input to get security advice
    }
  };
  ...
  const handleKeyPress = (event) => {
    if (event.key === "Enter") {
      fetchsecurityAdvice(); // Get security advice instead of direct speaking
    }
  };
  ...
  return (
    <main className="avatar-container">
      <h1 className="app-title">👨‍💼 AI security Advisor</h1>

      <div className="video-container">
        <video ref={videoRef} autoPlay playsInline className="avatar-video" />
        {!isSessionActive && (
          <div className="video-placeholder">
            <span>Start session to activate your security advisor</span>
          </div>
        )}
      </div>
